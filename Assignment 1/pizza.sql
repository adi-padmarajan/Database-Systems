...
FROM Serves AS s
JOIN Eats AS e ON s.pizza = e.pizza
...
FROM Frequents AS f
JOIN Person AS p ON p.name = f.name
WHERE p.age < 18
ORDER BY f.pizzeria, p.name;
...
JOIN Person    AS p1 ON p1.name = f1.name
WHERE p1.age < 18
  AND EXISTS (
        SELECT 1
        FROM Frequents AS f2
        JOIN Person    AS p2 ON p2.name = f2.name
        WHERE f2.pizzeria = f1.pizzeria
          AND p2.age > 30
  );
...
SELECT DISTINCT
  f1.pizzeria,
  p1.name  AS person1, p1.age AS age1,
  p2.name  AS person2, p2.age AS age2
FROM Frequents AS f1
JOIN Person    AS p1 ON p1.name = f1.name   AND p1.age < 18
JOIN Frequents AS f2 ON f2.pizzeria = f1.pizzeria
JOIN Person    AS p2 ON p2.name = f2.name   AND p2.age > 30
ORDER BY f1.pizzeria, person1, person2;
...
SELECT
  e.name,
  COUNT(DISTINCT e.pizza) AS pizza_types
FROM Eats AS e
GROUP BY e.name
HAVING COUNT(DISTINCT e.pizza) >= 2
ORDER BY pizza_types DESC, e.name ASC;
